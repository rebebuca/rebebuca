...
pub async fn add_project_detail_db(project_detail: Model) -> Result<Vec<Model>, DbErr> {
    ...
    let mut re = project_detail::Entity::find().all(&db).await?;
    let count = re.len();
    update_project_count_db(count, project_id).await?;
...
pub async fn del_project_detail_db(id: String) -> Result<Vec<Model>, DbErr> {
    let db: DatabaseConnection = connection::establish_connection().await.unwrap();
    project_detail::Entity::delete_by_id(id).exec(&db).await?;
    let mut result = project_detail::Entity::find().all(&db).await?;
